...
void loadIn(float* input, hls::stream<unroll1in> &outStream){
    ...
    for(int loopTime = 0; loopTime < TP_K; loopTime++){
	    ...
	    #pragma HLS PIPELINE
	    	for(int ii = 0; ii < P0; ii++){
	    		outStreamEle.a[loopTime][i * P0 + ii] = input[i * P0 + ii + loopTime * L0];	
	    		//cout << loopTime << endl;
...
void blockMul1(hls::stream<block1in> &inStream, VecL1* weight1, hls::stream<block1out> &outStream){
	...
	#pragma HLS ARRAY_PARTITION variable=outStreamEle.a cyclic factor=128   // P1
...
void act1(hls::stream<block1out> &inStream, const float* bias1, hls::stream<unroll2in> &outStream){
	...
	#pragma HLS ARRAY_PARTITION variable=inStreamEle.a cyclic factor=128			 // P1, less resources
	unroll2in outStreamEle;
	#pragma HLS ARRAY_PARTITION variable=outStreamEle.a dim=2 cyclic factor=128   // P1, directly compact
